...
/**
 * @wordpress-plugin
 * Plugin Name:       Albo Online
 * Plugin URI:        https://it.wordpress.org/
...
//        wp_enqueue_style( 'AlboPretorioNewStyle');
	...
	global $wpdb;
		...
		if($permProp==7 Or $permProp==6 Or $permProp==3 Or $permProp==2)
			...
			<table class="widefat" style="width:100%; margin-top:20px; border-collapse:collapse; border:1px solid #ddd;">
				<thead>
					<tr style="background-color:#eaeaea; border-bottom:1px solid #ddd;">
				...
				<tbody>
					<tr style="border-bottom:1px solid #ddd;">
						<td style="text-align:left;">' . __('Atti', 'albo-online') . '</td>
						<td>' . $n_atti . '</td>
						<td>' . $n_atti_dapub . '</td>
						<td>' . $n_atti_attivi . '</td>
						<td>' . $n_atti_storico . '</td>
						<td>' . $n_atti_oblio . '</td>
					</tr>
					<tr style="border-bottom:1px solid #ddd;">
						<td>' . __('Categorie', 'albo-online') . '</td>
						<td colspan="5">' . $n_categorie . '</td>
					</tr>
					<tr style="border-bottom:1px solid #ddd;">
						<td>' . __('Allegati', 'albo-online') . '</td>
						<td colspan="5">' . $n_allegati . '</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="widefat" style="margin-top:30px;">
			<h2 style="text-align:center;">' . __('Stato Plugin', 'albo-online') . '</h2>
			<table style="width:100%; border-collapse:collapse; border:1px solid #ddd; margin-top:20px;">
				...
				<tbody>
					<tr style="border-bottom:1px solid #ddd;">
						<td>' . __('Librerie', 'albo-online') . '</td>';
		if (is_file(Albo_DIR . '/inc/pclzip.php')) {
			echo '<td><span class="dashicons dashicons-yes" style="color:#18b908; font-size:1.5em;"></span></td>
				  <td></td>
				  <td></td>';
		} else {
			echo '<td><span class="dashicons dashicons-no" style="color:red; font-size:1.5em;"></span></td>
				  <td>' . __('Senza questa libreria non puoi eseguire i Backup', 'albo-online') . '</td>
				  <td></td>';
		}
		echo '</tr>
			  <tr style="border-bottom:1px solid #ddd;">
				  <td>' . __('Diritto all\'oblio', 'albo-online') . '</td>';
		if ($oblio && ap_VerificaRobots() && ap_VerificaOblio()) {
			echo '<td><span class="dashicons dashicons-yes" style="color:#18b908; font-size:1.5em;"></span></td>
				  <td></td>
				  <td></td>';
		} else {
			echo '<td><span class="dashicons dashicons-no" style="color:red; font-size:1.5em;"></span></td>
				  <td></td>
				  <td><a href="?page=Albo_Pretorio&amp;action=creafoblio&amp;rigenera=' . wp_create_nonce('rigeneraoblio') . '">' . __('Rigenera files', 'albo-online') . '</a></td>';
		}
		echo '</tr>
			  <tr style="border-bottom:1px solid #ddd;">
				  <td>' . __('Cartelle esistenza e permessi', 'albo-online') . '</td>';
		...
		echo '</tr>
			...
			<p style="text-align:center; margin-top:10px; font-style:italic;">' . __('per maggiori dettagli eseguire la verifica della procedura presente nel menu Utility', 'albo-online') . '</p>
		</div>';
		if (ap_get_num_categorie() == 0) {
			echo '<div class="welcome-panel" style="background-color: #F0F0F1; padding: 15px; border: 1px solid #ddd; margin-bottom: 15px;">
					<div class="widefat">
						<p style="text-align: center; font-size: 1.2em; font-weight: bold; color: green;">
						' . __('Non risultano categorie codificate, se vuoi posso impostare le categorie di default', 'albo-online') . ' &ensp;&ensp;<a href="?page=utilityAlboP&amp;action=creacategorie" style="color: #0073aa;">' . __('Crea Categorie di Default', 'albo-online') . '</a></p>
					</div>
				  </div>';
		}
		...
		if (ap_num_responsabili() == 0) {
			echo '<div class="welcome-panel" style="background-color: #F0F0F1; padding: 15px; border: 1px solid #ddd; margin-bottom: 15px;">
					<div class="widefat">
						<p style="text-align: center; font-size: 1.2em; font-weight: bold; color: green;">
						' . sprintf(__('Non risultano %sResponsabili%s codificati, devi crearne almeno uno prima di iniziare a codificare gli Atti', 'albo-online'), "<strong>", "</strong>") . ' &ensp;&ensp;<a href="?page=soggetti" style="color: #0073aa;">' . __('Crea Soggetti', 'albo-online') . '</a></p>
					</div>
				  </div>';
		}
		...
		if (ap_num_unitao() == 0) {
			echo '<div class="welcome-panel" style="background-color: #F0F0F1; padding: 15px; border: 1px solid #ddd; margin-bottom: 15px;">
					<div class="widefat">
						<p style="text-align: center; font-size: 1.2em; font-weight: bold; color: green;">
						' . sprintf(__('Non risulta nessuna %sUnità Organizzativa%s codificata, devi crearne almeno una prima di iniziare a codificare gli Atti', 'albo-online'), "<strong>", "</strong>") . ' &ensp;&ensp;<a href="?page=unitao" style="color: #0073aa;">' . __('Crea Unità Organizzativa', 'albo-online') . '</a></p>
					</div>
				  </div>';
		}
}	
